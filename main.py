...
def train():
    ...
    DQN.train(episodes=10000,
              episode_duration=5000,
              epsilon=(lambda x: max(1 - (x / 10000), 0.1)),
...
def run():
    ...
    DQN.Q.model.load_state_dict(torch.load('model_torch/last_trained_model.pth'))  # Load the saved weights
    DQN.Q.model.eval()  # Set the model to evaluation mode
